...
void allocate_memory(list_t *freelist, list_t *alloclist, int pid, int blocksize, int policy) {
    ...
    // Find the block based on the allocation policy
    if (policy == 1) { // FIFO
        while (current != NULL) {
...
void deallocate_memory(list_t *alloclist, list_t *freelist, int pid, int policy) {
    ...
    // Find the block to deallocate based on the pid
    while (current != NULL) {
    ...
    // Deallocate the block and add it back to the freelist
    blk->pid = 0;  // Set block as free
